...
//go:build (aix || darwin || dragonfly || freebsd || (!android && (linux || junction)) || netbsd || openbsd || solaris) && cgo && !osusergo
// +build aix darwin dragonfly freebsd !android,linux !android,junction netbsd openbsd solaris
// +build cgo
// +build !osusergo

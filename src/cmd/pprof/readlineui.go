...
//go:build (darwin || dragonfly || freebsd || (linux || junction) || netbsd || openbsd || solaris || windows) && !appengine && !android
// +build darwin dragonfly freebsd linux junction netbsd openbsd solaris windows
// +build !appengine
// +build !android

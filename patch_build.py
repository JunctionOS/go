...
def process_file(root, basename):
    ...
    try:
        ...
        for i, line in enumerate(lines):
            if not constraint_line_nr and line.startswith("//go:build "):
        ...
        if is_linux_ver:
            ...
            if constraint_line_nr is not None:
                ...
                writelines(file_path, ["//go:build !junction"] + lines)
                writelines(jname, ["//go:build junction"] + jlines)

            return


        if constraint_line_nr or plusbuild:
            writelines(file_path, lines)
